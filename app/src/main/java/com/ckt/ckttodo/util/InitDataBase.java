...
public class InitDataBase {
    ...
    public static void initData(final Context context) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            project.setProjectSummary(context.getResources().getString(R.string.init_project_summary));
            ...
            for (int j = 0; j < 4; j++) {
                ...
                DatebaseHelper.getInstance(context).getRealm().executeTransaction(new Realm.Transaction() {
                    ...
                    public void execute(Realm realm) {
                        Project sProject = DatebaseHelper.getInstance(context).getRealm().where(Project.class).equalTo(PROJECT_ID, projectId).findFirst();
                        Plan plan = new Plan();
                        plan.setPlanId(planId);
                        plan.setPlanName(context.getResources().getString(R.string.init_plan_name));
                ...
                if (j == 0) {
                    DatebaseHelper.getInstance(context).getRealm().executeTransaction(new Realm.Transaction() {
                        @Override
                        public void execute(Realm realm) {
                            Plan sPlan = DatebaseHelper.getInstance(context).getRealm().where(Plan.class).equalTo(PLAN_ID, planId).findFirst();
                            for (int j = 0; j < 4; j++) {
